...
class CKFinder
{
	...
	public function SetupFCKeditorObject( &$editorObj, $imageType = null, $flashType = null )
	{
		...
		$editorObj->Config['LinkUploadURL'] = '/' . urlencode( 'hcCKEditorUpload.php?command=QuickUpload&type=Files' ) ;
		$editorObj->Config['ImageUploadURL'] = '/' . urlencode( 'hcCKEditorUpload.php?command=QuickUpload&type=') . ( empty( $imageType ) ? 'Images' : $imageType ) ;
		$editorObj->Config['FlashUploadURL'] = '/' . urlencode( 'hcCKEditorUpload.php?command=QuickUpload&type=') . ( empty( $flashType ) ? 'Flash' : $flashType ) ;
	}
	...
	public function SetupCKEditorObject( &$editorObj, $imageType = null, $flashType = null )
	{
		...
		$dir = substr( $url, 0, strrpos( $url, "/" ) + 1 ) ;
		$editorObj->config['filebrowserUploadUrl'] = '/' . 'hcCKEditorUpload.php?command=QuickUpload&type=Files' ;
		$editorObj->config['filebrowserImageUploadUrl'] = '/' . 'hcCKEditorUpload.php?command=QuickUpload&type=' . ( empty( $imageType ) ? 'Images' : $imageType ) ;
		$editorObj->config['filebrowserFlashUploadUrl'] = '/' . 'hcCKEditorUpload.php?command=QuickUpload&type=' . ( empty( $flashType ) ? 'Flash' : $flashType ) ;
	}
}
