...
{       
    ...
    //在这里加入自己需要的判断逻辑，千万不要直接改成return true;这样相当于免费把自己的服务器变成一个网络硬盘+肉鸡，任何人都可以直接上传任何文件(包括木马)
    if(isset($_COOKIE['cka']) && md5($clientIp.md5($key))==$_COOKIE['cka']){
        return true;
    }else{
        return false;
    }
...
//        $host_names = array('backend.hoopchina.com', 'http://www.liangle.com/');
//        
//        if(in_array($_SERVER['HTTP_HOST'], $host_names)){
//            return file_get_contents('http://'.$_SERVER['HTTP_HOST'].'/backend.php/backend_user/loginCheck');
//        }
//        
//        var_dump(file_get_contents('http://'.$_SERVER['HTTP_HOST'].'/backend_dev.php/backend_user/loginCheck'));exit;
//	return file_get_contents('http://'.$_SERVER['HTTP_HOST'].'/backend_dev.php/backend_user/loginCheck') === 'yes' ? true : false;
...
$config['ResourceType'][] = Array(
		'name' => 'Images',
		'url' => 'http://c1.hoopchina.com.cn'.$baseUrl . 'images/'.date('Ymd').'/'.  date('H'),
		'directory' => $baseDir . 'images/'.date('Ymd').'/'.  date('H'),
		'maxSize' => "4M",
