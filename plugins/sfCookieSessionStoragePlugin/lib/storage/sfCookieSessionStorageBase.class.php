...
abstract class sfCookieSessionStorageBase extends sfSessionStorage {
    ...
    protected function compress($data) {
        ...
        if ($this->options['use_compression']) {
            return gzdeflate($data, self::COMPRESSION_LEVEL);
        }else{
            return $data;
        }
    }
    ...
    public function regenerate($destroy = false) {
        ...
        setcookie(session_id(), null, time() - 3600,$this->options['session_cookie_path'], $this->options['session_cookie_domain'], $this->options['session_cookie_secure'], $this->options['session_cookie_httponly']); 
