...
class sfCaptchaGD 
{
  ...
  function __construct() {
      ...
      $this->fontSize = sfConfig::get('app_sf_captchagd_font_size', "20");
      ...
      $this->image_height = sfConfig::get('app_sf_captchagd_image_height', 58);
  }
  ...
  public function generateImage($securityCode = NULL)
  {
    ...
    $rand_list =  (float) $sec + ((float) $usec * 100000);
    mt_srand($rand_list);
    $randval = mt_rand();
    $c_value = md5('voice_front' . time() .$randval);
    $r_key_str = md5($c_value . 'voice#@#hupu');
    $context->getResponse()->setCookie('front_captcha',$c_value,time()+60);
    $redis = $context->getDatabaseConnection('voiceRedis');
    $redis->setex($r_key_str,60,$this->securityCode);
