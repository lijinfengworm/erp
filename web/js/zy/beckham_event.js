...
var mainList = {
	init:function(){
			...
			var _value = boxreleaseForm.serialize(),action = boxreleaseForm.attr('action');
                 $.post(action, _value, function (data) {
                    if (data.status == 0) {
                    	...
                    	window.location.href = "http://zy.hupu.com/memorywall/beckham-retire?uid="+login_uid+"&release="+login_uid;
		...
		this.pop_liang.bind("click",function(){
			...
			$.post("http://zy.hupu.com/beckham/reply_light",{id:that.pop.find(".num").html()},function(data){
		...
		that.loadOver = false;
		this.l = items.length;    
		if(this.l<500)this.loadOver = true;
		this.c = 0;
	...
	setLoad:function(){
		...
		that.loading.show();
		that.loadeding = true;
		$.getJSON("http://zy.hupu.com/beckham/get_by_page",{page:that.loadC},function(data){
			that.create(data);
			if(data.length<500){
				that.loadOver = true;
			}
...
function shareSina () {
			var e = {
				title: curr_memory.content+"——"+curr_memory.username+"留于 #虎扑阵营# #再见，贝克汉姆纪念墙#。点击这里写下你想对小贝说的话：",
                url: "http://zy.hupu.com/memorywall/beckham-retire?uid="+curr_memory.hupu_uid,
                pic: "http://i1.hoopchina.com.cn/u/1305/17/602/15669602/3cb0366abig.jpeg",
                summary: "",
                type: ""
			};
               var o = 600,
