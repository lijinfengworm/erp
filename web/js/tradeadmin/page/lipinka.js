...
$(function(){
    ...
    //选择申请类型  页面跳转
    $("input:radio[name='trd_lipinka[type]']").on('change',function(e){
        //取消事件的默认动作
        e.preventDefault();
        //终止事件 不再派发事件
        e.stopPropagation();
        ...
        Wind.use('artDialog', function () {
            dialog({
                ...
                cancel: function () {
                    var  curr = (_val == 1) ? 1 : 0;
                    $("input:radio[name='trd_lipinka[type]']:eq("+curr+")").prop("checked",1);
                    $("input:radio[name='trd_lipinka[type]']:eq("+(_val-1)+")").removeAttr("checked");
                }
            }).showModal();
        });
