...
var orderConfirm = {
    ...
    bindFun:function(){
       ...
       $(".new-address").find(".save").click(function(){
           ...
           $.get("http://www.shihuo.cn/haitao/saveDeliveryAddress",{name:nameVal,identity_number:cardVal,mobile:mobileVal,phonesection:phonesectionVal,phonecode:phonecodeVal,phoneext:phoneextVal,province:provinceVal,city:cityVal,area:areaVal,street:streetVal,postcode:postcodeVal,defaultflag:defaultflagVal},function(data){
                if(data.status*1 == 1){
                    $this.tips(data.msg);
                }else{
...
var submit = {
   ...
   bindFun:function(){
      ...
      $(".submit-btn").click(function(){
          ...
          $this.val("提交中...");          
          $.post("http://www.shihuo.cn/haitao/cartSubmitOrder",{address_id:priceValue.address_id,remark:priceValue.remark,data:priceValue.data,coupon_id:priceValue.coupon_id},function(data){
                if(data.status*1 == 0){
                    __dace.sendEvent('shihuo_daigou_orderpay_cart_'+data.data.orderSn);
                     setTimeout(function(){
      ...
      $(".coupon-box").find("li").click(function(){
           ...
           $.post("http://www.shihuo.cn/haitao/cartConfirmLipinka",{data:priceValue.data,coupon_id:couponId},function(data){
                if(data.status*1 == 0){
                    if($this.hasClass('on')){
                        $("#price_up").html("￥"+data.data.total_price);
                        $("#price_total").html("￥"+data.data.original_total_price);
                        $this.removeClass('on');
                        //$this.find("input").attr("checked","false").hide()
                    }else{
                        $this.attr("check") == "1";
                        $("#price_up").html("￥"+data.data.total_price);
                        $("#price_total").html("￥"+data.data.original_total_price);
                        ...
                        //$this.find("input").attr("checked","checked").show();
                    }
