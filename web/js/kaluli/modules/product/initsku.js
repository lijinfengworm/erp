define(function() {
    ...
    initsku.prototype = {
        defaults: {
            sort: ".sku-content",
            sortelem: ".sku",
            kaluliprice: "#kaluliPrice",//卡路里价格
            originalPrice: "#originalPrice",//参考价格
            stock: "#stock",//库存
            storehouse_name:"#delivery_area",//发货地
            discount:"#kaluli_discount",//单件打折
            select: "cur",
            unbindselect:'unclick',
            count_downId:'#count_box',
            discount_rate:1
        },
        init: function() {
            var t = this;
            ...
            defaultDiscount = +$(t.defaults.discount).val()/10;
            ...
            $(t.defaults.sortelem).each(function() {
                ...
                if (selectedObjs.length) {
                    ...
                    $(".numbox-text").attr("maxNum",stocks) //获取商品最大数量
                    ...
                    $(t.defaults.kaluliprice).text((discountmaxPrice * defaultDiscount).toFixed(2));
                    $('#pre_discounted_price').text('¥'+discountmaxPrice.toFixed(2));//折扣前的价格
        ...
        count_down: function () {
            var t = this,
                leftSec = $(t.defaults.count_downId).attr('leftSec');
            if(leftSec >= 0) {
