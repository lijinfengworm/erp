...
class ItemAllListUrlUtil {
     ...
     */
    public function getNewUrl($prefix,$name,$val,$type=1){
        ...
        if ($type == 1){
            ...
            if (isset($curr_params['tag'])) $where['tag'] = $curr_params['tag'];
            if (isset($curr_params['sex'])) $where['sex'] = $curr_params['sex'];
            ...
            if (isset($curr_params['w'])) $where['w'] = $curr_params['w'];
            $curr_params = $where;
        } else {
        ...
        if (isset($curr_params['sex'])){
            $postfix .= '-'.$curr_params['sex'];
            unset($curr_params['sex']);
        } else {
            $postfix .= '-0';
        }
        $postfix = rtrim($postfix,'-0');
        $postfix = rtrim($postfix,'-0');
        $postfix = rtrim($postfix,'-0');
        if ($postfix){
            $return .= '/p'.$postfix;
     ...
     * @param int $type 1表示增加 0 表示删除  
     */
    public function getGrouponNewUrl($prefix,$name,$val,$type=1){
        $curr_params = $this->curr_params;
        if ($type == 1){
            $where = array();
            if($name) $curr_params[$name] = $val;
     ...
     * @param int $type 1表示增加 0 表示删除
     */
    public function getDaigouNewUrl($prefix,$name,$val,$type=1){
        $curr_params = $this->curr_params;
        ...
        if ($type == 1){
            ...
            if($name) $curr_params[$name] = $val;
            if (isset($curr_params['brand'])) $where['brand'] = $curr_params['brand'];
            if (isset($curr_params['root_name'])) $where['root_name'] = $curr_params['root_name'];
            if (isset($curr_params['children_name'])) $where['children_name'] = $curr_params['children_name'];
            if (isset($curr_params['price'])) $where['price'] = $curr_params['price'];
            if (isset($curr_params['order'])) $where['order'] = $curr_params['order'];
            if (isset($curr_params['page']) &&  $curr_params['page'] > 1) $where['page'] = $curr_params['page'];
            if (isset($curr_params['keywords'])) $where['keywords'] = $curr_params['keywords'];
            if (isset($curr_params['aid'])) $where['aid'] = $curr_params['aid'];
            $curr_params = $where;
        } else {
            unset($curr_params[$name]);
        }
        $return  = $prefix;
        $postfix = '';
        if (isset($curr_params['root_name'])){
            $postfix .= '-'.$curr_params['root_name'];
            unset($curr_params['root_name']);
        } else {
            $postfix .= '-0';
        }
        if (isset($curr_params['children_name'])){
            $postfix .= '-'.$curr_params['children_name'];
            unset($curr_params['children_name']);
        } else {
            $postfix .= '-0';
        }
        if (isset($curr_params['brand'])){
            $postfix .= '-'.urlencode(urlencode(FunBase::escapeSpecialCharacters( $curr_params['brand'] )));
            unset($curr_params['brand']);
        } else {
            $postfix .= '-0';
        }

        if(isset($curr_params['keywords'])){
            $curr_params['keywords'] = FunBase::escapeSpecialCharacters( $curr_params['keywords'] );
        }
     ...
     * @param int $type 1表示增加 0 表示删除
     */
    public function getFindNewUrl($prefix,$name,$val,$type=1){
        $curr_params = $this->curr_params;
        if(isset($curr_params['root_id']))unset($curr_params['root_id']);
