...
class kllVerify {
    ...
    public function check($code, $id = '') {
        ...
        // 验证码不能为空
        $secode = json_decode(FunBase::SiteCookie($key),true);
        if(empty($code) || empty($secode)) {
            return false;
        }
        // session 过期
        if($_SERVER['REQUEST_TIME'] - $secode['verify_time'] > $this->expire) {
            FunBase::SiteCookie($key,null,array('domain'=>$this->config['domain']));
            return false;
        }
    ...
    public function entry($id = '') {
        ...
        FunBase::SiteCookie($key.$id, json_encode($secode),array('domain'=>$this->config['domain']));
