...
/**
 * Class cpsKaluliService
 * version: 1.0
 */
class cpsKaluliService extends kaluliService
{
    ...
    public function executeOrderList()
    {
        ...
        $status = trim($this->getRequest()->getParameter("status",""));
...
//        }
        ...
        $cpsorderObj = KllCpsOrderTable::getInstance()->createQuery()->where('union_id = ?', $union_id); //->andWhere('test = ?', 0)
        if ($sn) {
            $cpsorderObj->andWhere('order_number = ?', $sn);
        } else {
            if($status == 1) {
                $cpsorderObj->andWhere("status = 0"); //新创建订单未支付
            } elseif ($status == 2) {
                $cpsorderObj->andWhere("status in (1,2,3)");//有效订单包括已支付,已发货,有效
            } elseif ($status == 3) {
                $cpsorderObj->andWhere("status in (4,5)");//无效订单为无效,退货
            }
