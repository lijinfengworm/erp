 ...
 * @author     wangpeng
 */
class sfSwooleRequest extends sfWebRequest
{
    const
        PORT_HTTP  = 80,
        PORT_HTTPS = 443;
    ...
    protected
        $swooleRequest = null,
        $swooleRequestHeader = null,
        $swooleRequestServer = null,
        $swooleRequestGet = null,
        $swooleRequestPost = null,
        $swooleRequestCookie = null,
        $swooleRequestFiles = null,
        $swooleRequestRawContent = null;
    protected $serverVarPrefix = array('host', 'connection', 'accept', 'user-agent', 'accept-encoding', 'accept-language', 'accept-charset', 'referer');
    /**
     * Class constructor.
     *
     * @see initialize()
     */
    public function __construct(sfEventDispatcher $dispatcher, $parameters = array(), $attributes = array(), $options = array(),$swooleRequest = array())
    {
        $this->swooleRequest = $swooleRequest;
        $this->swooleRequestHeader = $swooleRequest->header;
        $this->swooleRequestServer = $swooleRequest->server;

        foreach ($this->swooleRequestHeader as $i => $val) {
            if (in_array($i, $this->serverVarPrefix)) {
                $key = strtoupper('http_' . str_replace('-', '_', $i));
                if (!isset($this->swooleRequestServer[$key])) {
                    $this->swooleRequestServer[$key] = $val;
                }
            }
        }
        $this->swooleRequestGet = isset($swooleRequest->get)?$swooleRequest->get:array();
        $this->swooleRequestPost = isset($swooleRequest->post)?$swooleRequest->post:array();
        $this->swooleRequestCookie = isset($swooleRequest->cookie)?$swooleRequest->cookie:array();
        $this->swooleRequestFiles = isset($swooleRequest->files)?$swooleRequest->files:array();
        $this->swooleRequestRawContent = isset($swooleRequest->rawContent)?$swooleRequest->rawContent:array();
    ...
    public function initialize(sfEventDispatcher $dispatcher, $parameters = array(), $attributes = array(), $options = array())
    {
        ...
        sfRequest::initialize($dispatcher, $parameters, $attributes, $options);

        // GET parameters
        $this->getParameters = get_magic_quotes_gpc() ? sfToolkit::stripslashesDeep($this->swooleRequestGet) : $this->swooleRequestGet;
        $this->parameterHolder->add($this->getParameters);

        $postParameters = $this->swooleRequestPost;

        if (isset($this->swooleRequestServer['REQUEST_METHOD']))
        {
            switch ($this->swooleRequestServer['REQUEST_METHOD'])
            {
                case 'GET':
                ...
                case 'POST':
                    if (isset($this->swooleRequestPost['sf_method']))
                    {
                        $this->setMethod(strtoupper($this->swooleRequestPost['sf_method']));
                        unset($postParameters['sf_method']);
                    }
                    elseif (isset($this->swooleRequestGet['sf_method']))
                    {
                        $this->setMethod(strtoupper($this->swooleRequestGet['sf_method']));
                        unset($this->swooleRequestGet['sf_method']);
                    }
                    else
                    {
    ...
    public function getPathInfo()
    {
        ...
        // for IIS
        if (isset($this->swooleRequestServer['SERVER_SOFTWARE']) && false !== stripos($this->swooleRequestServer['SERVER_SOFTWARE'], 'iis') && $pos = stripos($pathInfo, '.php'))
        {
    ...
    public function getLanguages()
    {
        ...
        if (!isset($this->swooleRequestServer['HTTP_ACCEPT_LANGUAGE']))
        {
            return array();
        }

        $languages = $this->splitHttpAcceptHeader($this->swooleRequestServer['HTTP_ACCEPT_LANGUAGE']);
    ...
    public function getCharsets()
    {
        ...
        if (!isset($this->swooleRequestServer['HTTP_ACCEPT_CHARSET']))
        {
            return array();
        }

        $this->charsets = $this->splitHttpAcceptHeader($this->swooleRequestServer['HTTP_ACCEPT_CHARSET']);
    ...
    public function getAcceptableContentTypes()
    {
        ...
        if (!isset($this->swooleRequestServer['HTTP_ACCEPT']))
        {
            return array();
        }

        $this->acceptableContentTypes = $this->splitHttpAcceptHeader($this->swooleRequestServer['HTTP_ACCEPT']);
    ...
    public function getCookie($name, $defaultValue = null)
    {
        ...
        if (isset($this->swooleRequestCookie[$name]))
        {
            $retval = get_magic_quotes_gpc() ? sfToolkit::stripslashesDeep($this->swooleRequestCookie[$name]) : $this->swooleRequestCookie[$name];
        }
    ...
    public function getPathInfoArray()
    {
        if (!$this->pathInfoArray)
        {
            ...
            {
                case 'SERVER':
                    $this->pathInfoArray =& $this->swooleRequestServer;
                    break;
    ...
    public function getFiles($key = null)
    {
        ...
        {
            $this->fixedFileArray = self::convertFileInformation($this->swooleRequestFiles);
        }
