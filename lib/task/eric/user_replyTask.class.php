...
class user_replyTask extends sfBaseTask
{
  protected function configure()
  {
    ...
    $this->namespace        = 'eric';
    $this->name             = 'user_reply';
    $this->briefDescription = '';
    $this->detailedDescription = <<<EOF
The [eric:user_reply|INFO] task does things.
Call it with:
    user_reply
  [php symfony user_reply|INFO]
EOF;
  }
  ...
  protected function execute($arguments = array(), $options = array())
  {
    ...
    $this->log('start');
    $this->lockDir = '/tmp';
    $this->lockFile = 'autoRun_reply.lock';
    //加锁
    if(!$this->enterLock())
    {
        exit();
    ...
    $data = unserialize(file_get_contents(__DIR__.'/all_reply.txt'));
    ...
    $memcache = sfContext::getInstance()->getDatabaseConnection('tradeMemcache');
        ...
        foreach ($team_info['tids'] as $tid)
        {
            ...
            foreach ($infos as $i)
            {
                ...
                $info['object_name'] = $team_info[$tid]['name'];
                //print_R($info);
                $this->addLog($info);
            }
            continue;
            
            $tids = fbd_personTable::getInstance()->getSomeThreads($tid, strtotime('2012-12-20'),  strtotime('2013-03-12'));
            $a = 0;
            $tid_array = array();
            foreach  ($tids as $i)
            {
                $replys = fbd_personTable::getInstance()->getThreadMsg($i['tid'], strtotime('2012-12-20'),  strtotime('2012-12-28'));
                
                foreach ($replys as $reply)
                {
                    $reply['author'] = mb_convert_encoding($reply['author'], 'utf-8', 'gbk');
                    $date = date('Y-m-d',$reply['postdate']);
                    if(empty($tid_array['date'][$date][$reply['authorid']]))
                    {
                        if(!isset($tid_array['user'][$reply['authorid']]['count']))
                        {
                            $tid_array['user'][$reply['authorid']]['username'] = $reply['author'];
                            $tid_array['user'][$reply['authorid']]['count'] = 0;
                        }else
                        {
                            $tid_array['user'][$reply['authorid']]['count'] = $tid_array['user'][$reply['authorid']]['count']+1;
                            if($tid_array['user'][$reply['authorid']]['count'] >= 3 && empty($tid_array['user_log'][$reply['authorid']]))
                            {
                                $result[$tid][] = array('uid'=>$reply['authorid'],'username'=>$reply['author']);
                                $tid_array['user_log'][$reply['authorid']] = 1;
                            }
                        }
                        
                        
                        $tid_array['date'][$date][$reply['authorid']] = 1;
                        
                        //$this->log('add---'.$reply['tid'].'---'.$reply['fid'].'--'.$reply['author'].'---'.$reply['authorid'].'---'.$date);
                    }else{
                        
                        //$this->log('ccc---'.$reply['tid'].'---'.$reply['fid'].'--'.$reply['author'].'---'.$reply['authorid'].'---'.$date);
                    }
                }
...
//            $this->log($tid_array['result']);
             
            //$this->log(count($tids));
            continue;
        }
        file_put_contents('/tmp/all_reply.txt', serialize($result));
  }
  function addLog($info) {
    ...
    private function getTeamInfo() {
        ...
        */
        $teams = array();
       foreach ($teamsInfoConfig as $k => $v)
       {
           foreach ($v as $v2)
           {
               if(!empty($v2['fid']))
               {
                 $teams['tids'][] = $v2['fid'];
                 $teams[$v2['fid']] = array('fid'=>$v2['fid'],'name'=>$v2['name']);
               }
           }
       }
       return $teams;
