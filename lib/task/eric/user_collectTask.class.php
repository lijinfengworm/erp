...
class user_collectTask extends sfBaseTask
{
  protected function configure()
  {
    ...
    $this->namespace        = 'eric';
    $this->name             = 'user_collect';
    $this->briefDescription = '';
    $this->detailedDescription = <<<EOF
The [eric:user_collect|INFO] task does things.
Call it with:
    user_collect
  [php symfony user_collect|INFO]
EOF;
  }
  ...
  protected function execute($arguments = array(), $options = array())
  {
    ...
    $this->log('start');
    $this->lockDir = '/tmp';
    $this->lockFile = 'autoRun_collect.lock';
    //加锁
    if(!$this->enterLock())
    {
        exit();
    ...
    $memcache = sfContext::getInstance()->getDatabaseConnection('tradeMemcache');
        ...
        {
            $memcache_key = "user_collect_tid_".$tid;
            ...
            $this->log('start:'.$team_info[$tid]['name']);
            while (1)
            {
                
                $rs = fbd_personTable::getInstance()->getSomeUserForum($start,$tid, $limit);
                ...
                foreach ($rs as $r)
                {
                    $info['action'] = "subscribe_forum";
                    $info["userid"] = $r["uid"];
                    $info["username"] = mb_convert_encoding($r["username"], 'utf-8', 'gbk');
...
//                    print_r($info);
            ...
            $this->log('success '.$team_info[$tid]['name']);
        }
