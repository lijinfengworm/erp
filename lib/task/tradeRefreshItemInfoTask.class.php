...
class tradeRefreshsoldcountTask extends sfBaseTask
{
  protected function configure()
  {
    ...
    $this->name             = 'refresh-iteminfo';
    $this->briefDescription = '';
    $this->detailedDescription = <<<EOF
The [trade:refresh-iteminfo|INFO] task does things.
Call it with:
...
EOF;
  ...
  protected function execute($arguments = array(), $options = array())
  {
    ...
    $lastId = $ttserver->get('refreshiteminfolastid');

    if(!empty($lastId))
    {
        $items = TrdItemTable::getInstance()->getItemsForRefresh(300, $lastId,0);
    }else{
        $items = TrdItemTable::getInstance()->getItemsForRefresh(300, 0,1);
    }
    ...
    if(count($items) > 0)
    {
        foreach ($items as $item)
        {
            if(!$item->getShopId())
            {
                $this->log( $item->getId().'  isn\'t taobao continue');
                continue;
            }           
...
//        $item->setGiveMoney($give_money);
            ...
            $ttserver->set('refreshiteminfolastid',$item->getId(),0,86400);
        }
    }else{
        $ttserver->set('refreshiteminfolastid',0);
    }
