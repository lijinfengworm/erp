 ...
 */
class TrdMenuTable extends Doctrine_Table
{
    /**
     * Returns an instance of this class.
     *
     * @return object TrdMenuTable
     */
    public static function getInstance()
    {
        return Doctrine_Core::getTable('TrdMenu');
    }
    ...
    public function getAllMenus(){
        $objs = self::getInstance()->createQuery('a')
            ->select('id,root_id,name,level')
            ->execute();
        ...
        foreach ($objs as $rek=>$rev) {
            if($rev->getLevel() == 0){
                array_push($one_level,$rev->getId());
                unset($objs[$rek]);
            }
        }
        ...
        foreach($one_level as $k=>$v){
            foreach ($objs as $rek=>$rev) {
                if($rev->getRootId() == $v){
                    $all_menu[$v][$rev->getId()] = $rev->getName();
                    unset($objs[$rek]);
                }
            }
        }
