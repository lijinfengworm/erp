...
class ga {
    ...
    public function __construct($gaAccount) {
        $this->utmac =  $gaAccount;
        ...
        $this->utmr = urlencode($referer);

        if (!empty($path)) {
            $this->utmp =  urlencode($path);
        }
         // Tracker version.
    ...
    private function getIP($remoteAddress) {
    ...
    private function writeGifData() {
        header("Content-Type: image/gif");
        header("Cache-Control: " .
                "private, no-cache, no-cache=Set-Cookie, proxy-revalidate");
        header("Pragma: no-cache");
        header("Expires: Wed, 17 Sep 1975 21:32:10 GMT");
        echo join($this->GIF_DATA);
    }
    ...
    private function sendRequestToGoogleAnalytics($utmUrl) {
        $options = array(
            "http" => array(
                "method" => "GET",
    ...
    public  function trackPageView() {
        ...
        $guidHeader = isset($_SERVER["HTTP_X_DCMGUID"])?$_SERVER["HTTP_X_DCMGUID"]:NULL;
        if (empty($guidHeader)) {
            $guidHeader = isset($_SERVER["HTTP_X_UP_SUBNO"])?$_SERVER["HTTP_X_UP_SUBNO"]:NULL;
        }
        if (empty($guidHeader)) {
            $guidHeader = isset($_SERVER["HTTP_X_JPHONE_UID"])?$_SERVER["HTTP_X_JPHONE_UID"]:NULL;
        }
        if (empty($guidHeader)) {
            $guidHeader = isset($_SERVER["HTTP_X_EM_UID"])?$_SERVER["HTTP_X_EM_UID"]:NULL;
        }

        $visitorId = $this->getVisitorId($guidHeader, $account, $userAgent, $cookie);
        ...
        $utmUrl = $utmGifLocation . "?" .
                ...
                "&utmvid=" . $visitorId .
                "&utmip=" . $this->getIP(cdn2clientip::getIp());
