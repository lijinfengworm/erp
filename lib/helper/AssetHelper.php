...
function _compute_public_path($source, $dir, $ext, $absolute = false)
{
  ...
  {
    if(sfConfig::get('app_style_version_bak')){
        return $source.$query_string.(empty($query_string)?'?':'&').'v='.sfConfig::get('app_style_version');
    }else{
        //由前端控制版本号，kworm
        $style_version = date("Ymd");
        $file = sfConfig::get('sf_root_dir').'/../kaluli_web/style_version.json';
        $version_json = file_get_contents($file);
        if(!empty($version_json)){
          $version_json_obj = json_decode($version_json);
          $style_version = $version_json_obj->style_version;
        }
        return $source.$query_string.(empty($query_string)?'?':'&').'?v='.$style_version;
  ...
  //*****Hack By Scourgen Start
  
  if(sfConfig::get('app_style_version')){
        return $source.$query_string.(empty($query_string)?'?':'&').'v='.sfConfig::get('app_style_version');
  }else{
        return $source.$query_string.(empty($query_string)?'?':'&').'v='.date('Ymd');
  }
  
  //*****Hack By Scourgen End
}
