...
abstract class sfApplicationConfiguration extends ProjectConfiguration
{
  ...
  public function getTemplateDirs($moduleName)
  {
    ...
    $dirs[] = $this->getKaluliSymfonyTemplateDir($moduleName);                 // application
  ...
  /**
   * 卡路里前后端分离，把template移到kaluli_web目录下
   */
  public function getKaluliSymfonyTemplateDir($moduleName){
    if($this->application == 'kaluli'){
      return $this->getRootDir().'/../kaluli_web/kaluli_pc/templates/kaluli/'.$moduleName.'/templates';
    }else{
