...
class emoji
{
	public $emojiMap = array(
		'names' => array(
			...
			//ios6后新增的表情，用Simple HTML DOM parser 采集自http://punchdrunker.github.io/iOSEmoji/table_html/ios6/index.html
			"\\xF0\\x9F\\x98\\x84"=> '',
			"\\xF0\\x9F\\x98\\x83"=> '',
			"\\xF0\\x9F\\x98\\x80"=> '',
			"\\xF0\\x9F\\x98\\x8A"=> '',
	...
	public function emoji_convert($text, $map){
		...
		return str_replace(array_keys($this->emojiMap[$map]), $this->emojiMap[$map], $text);
	}

	public function emoji_get_name($unified_cp){

		return $this->emojiMap['names'][$unified_cp] ? $this->emojiMap['names'][$unified_cp] : '?';
	}
	...
	public function emoji_to_string($text, $replaceStr = '') {
		$text = str_ireplace(array_keys($this->emojiMap['names']), $this->coverString($replaceStr), $this->coverString($text));
	 ...
	 * @param unknown_type $text
	 */
	public function coverString($text) {
		$text=urlencode($text);
		$text=str_replace("%", "\\x", $text);
		return $text;
	}
